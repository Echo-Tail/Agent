...
export function submitLogApi(req: SubmitLogRequest) {
  return http.post<ApiResponse<SystemLogDTO>>('/system-logs', req)
}
...
export function queryLogsApi(params: {
  ...
  return http.get<ApiResponse<LogPageDTO>>('/system-logs', {
    params: {
      page: params.page ?? 0,
      size: params.size ?? 20,
...
export function getLogStatsApi() {
  return http.get<ApiResponse<LogStatsDTO>>('/system-logs/stats')
}

export function clearLogsApi() {
  return http.delete<ApiResponse<null>>('/system-logs')
}
