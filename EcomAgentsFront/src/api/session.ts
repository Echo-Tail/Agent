...
export function listSessionsApi(params?: { folderId?: number; agentId?: number }) {
  return http.get<ApiResponse<SessionSummary[]>>('/sessions', { params })
}

export function getSessionApi(id: number) {
  return http.get<ApiResponse<Session>>(`/sessions/${id}`)
}

export function createSessionApi(data: { agentId: number; title?: string; folderId?: number }) {
  return http.post<ApiResponse<Session>>('/sessions', data)
}

export function updateSessionApi(id: number, data: { title?: string; folderId?: number | null }) {
  return http.put<ApiResponse<Session>>(`/sessions/${id}`, data)
}

export function deleteSessionApi(id: number) {
  return http.delete<ApiResponse<void>>(`/sessions/${id}`)
}

/* ====== Messages ====== */

export function addMessageApi(sessionId: number, role: string, content: string) {
  return http.post<ApiResponse<unknown>>(`/sessions/${sessionId}/messages`, { role, content })
}
...
export function updateFolderApi(id: number, data: { name: string }) {
  return http.put<ApiResponse<SessionFolder>>(`/session-folders/${id}`, data)
}

export function deleteFolderApi(id: number) {
  return http.delete<ApiResponse<void>>(`/session-folders/${id}`)
}
...
export async function streamChat(
  ...
  try {
    ...
    // Fallback: stream closed without a clean done/error event
    if (!completed) {
      clearTimeout(timeoutId)
      if (fullText) {
        onDone(fullText)
    ...
    if (timedOut) return // onError already called
    // Silently ignore user-initiated abort (stop button)
