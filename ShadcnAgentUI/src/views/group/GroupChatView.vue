...
function renderContent(text: string): string {
  ...
  let result = text.replace(
    /\[([^\]]+)\]\((https?:\/\/[^\s)]+|\/[^\s)]+)\)/g,
    `<a href="$2" target="_blank" class="text-primary underline hover:no-underline" download>$1</a>`
  )
  // 再处理 @提及
...
function getMemberAvatar(msg: GroupMessage) {
  return memberAvatars.value.get(`${msg.senderType}-${msg.senderId}`)
}
...
<template>
  <div class="flex h-[calc(100vh-7rem)] px-4 md:px-6">
    <!-- Main chat area -->
    <div class="flex-1 flex flex-col">
      <!-- Header -->
      <div class="flex shrink-0 items-center gap-3 border-b h-14">
        ...
        <Button variant="outline" size="sm" @click="showInviteDialog = true">
          <UserPlus class="mr-1 h-4 w-4" /> 邀请
        </Button>
      ...
      <AgentChatContainer class="flex-1 bg-muted/30" viewport-class="px-4 pb-4">
        ...
        <template v-else-if="messages.length === 0">
          <MessageScrollerItem message-id="empty">
            <div class="flex flex-col items-center justify-center py-20 text-muted-foreground">
              <Bot class="mb-3 h-12 w-12 opacity-30" />
              <p>群聊已创建，开始聊天吧</p>
              <p class="mt-1 text-xs">使用 @Agent名称 可以艾特群里的 Agent</p>
            </div>
          </MessageScrollerItem>
        </template>
        ...
        <template v-else>
          <MessageScrollerItem
            ...
            :scroll-anchor="isMyMessage(msg)"
          >
            <Message :align="isMyMessage(msg) ? 'end' : 'start'">
              <MessageAvatar class="self-start mt-6">
                <Avatar class="h-8 w-8">
                  <AvatarImage
                    v-if="getMemberAvatar(msg)?.avatar"
                    :src="getMemberAvatar(msg)!.avatar!"
                    :alt="getMemberAvatar(msg)?.name || 'avatar'"
                  />
                  <AgentIcon
                    v-else-if="msg.senderType === 'AGENT'"
                    :icon="getMemberAvatar(msg)?.icon"
                    class="h-5 w-5"
                  />
                  <AvatarFallback v-else class="text-xs">
                    {{ getMemberAvatar(msg)?.name?.charAt(0)?.toUpperCase() || '?' }}
                  </AvatarFallback>
                </Avatar>
              </MessageAvatar>
              <MessageContent>
                ...
                <div
                  ...
                  :class="isMyMessage(msg) ? 'justify-end' : ''"
                >
                ...
                <!-- Message content -->
                <Bubble :variant="isMyMessage(msg) ? 'default' : 'muted'">
                  <BubbleContent>
                    <MarkdownRenderer
      ...
      <div class="border-t pt-4 pb-2">
        <div class="flex gap-2">
          ...
          <Button size="icon" variant="outline" class="relative h-[44px] w-[44px] shrink-0" :disabled="fileUploading" :title="$t('common.upload')">
            <input id="group-file-upload" ref="fileInputRef" type="file" multiple class="absolute inset-0 cursor-pointer opacity-0" @change="handleFileUpload" />
    ...
    <!-- Members sidebar -->
    <div v-if="showMembers" class="w-72 shrink-0 overflow-y-auto border-l p-4 space-y-4">
      <div>
        <h3 class="mb-2 text-sm font-semibold">成员 ({{ members.length }})</h3>
        <div v-for="m in memberList" :key="m.id" class="flex items-center gap-3 py-1.5">
          <Avatar class="h-8 w-8 shrink-0">
            <AvatarImage v-if="m.avatar" :src="m.avatar" :alt="m.name || 'avatar'" />
            <AgentIcon
              v-else-if="m.memberType === 'AGENT'"
              :icon="m.icon"
              :avatar="m.avatar"
              class="h-5 w-5"
            />
          ...
          <div class="flex min-w-0 flex-1 items-center">
            <p class="mr-2 truncate text-sm">{{ m.name }}</p>
          </div>
          ...
          <Badge v-if="m.role === 'CREATOR'" variant="secondary" class="shrink-0 text-xs">创建者</Badge>
          <Badge v-else-if="m.memberType === 'AGENT'" variant="outline" class="shrink-0 bg-primary/5 text-xs">Agent</Badge>
        </div>
      </div>
    </div>
  </div>
